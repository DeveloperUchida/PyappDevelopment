...
# グローバル変数: 大きさが9x9で地雷の数が10個または大きさが11x11で地雷の数が12
ROWS, COLS, NUM_MINES = 9, 9, 10  # デフォルトの値
...
def main():
    ...
    global ROWS, COLS, NUM_MINES  # グローバル変数を使用
    if (ROWS == 9 and COLS == 9 and NUM_MINES == 10) or (ROWS == 11 and COLS == 11 and NUM_MINES == 12):
        game = Minesweeper(root, ROWS, COLS, NUM_MINES)
    else:
        messagebox.showerror("Invalid Configuration", "Please set the configuration to either 9x9 with 10 mines or 11x11 with 12 mines.")
        sys.exit(1)  # 終了コードを設定して終了
    root.mainloop()
